...
Logged bandit data: contexts \(x_i\), actions \(a_i \sim \pi_b(\cdot|x_i)\),
rewards \(r_i\), propensities \(\pi_b(a_i|x_i)\).

Learned policy \(\pi_\theta(a|x)\) (softmax over CF embeddings).
...
|------|-------------|--------------|
| Off-policy (OPC) | \(w_i = \pi_\theta(a_i|x_i)/\pi_b(a_i|x_i)\) | `propensity_mode="logged"` |
...
\[
\mathcal{L}_{\text{kl\_crm}}
=
-\widehat{\mathrm{SNDR}}_{\text{log}}
...
\[
\begin{aligned}
\widehat{\mathrm{SNDR}}_{\text{log},i}
&=
\underbrace{\frac{w_i^{\perp}}{\overline{w^\perp}}(r_i-\hat q_i)\,\log \pi_\theta(a_i|x_i)}_{\text{correction}}
+
\underbrace{\sum_a \hat q(x_i,a)\,\pi_\theta^\perp(a|x_i)\,\log \pi_\theta(a|x_i)}_{\text{DM}}
\end{aligned}
\]
...
\[
-\widehat{\mathrm{SNDR}}_{\text{log}} = -\frac{1}{n}\sum_i \widehat{\mathrm{SNDR}}_{\text{log},i}.
\]
...
=
\frac{1}{n}\sum_i
\bigl(\log \pi_b(a_i|x_i) - \log \pi_\theta(a_i|x_i)\bigr)
\approx
\mathbb{E}_{a\sim\pi_b}\bigl[\mathrm{KL}(\pi_b\|\pi_\theta)\bigr]
\text{ (single-action MC)}.
\]
...
| `crm` | clipped IPS risk \(-\overline{r\cdot\mathrm{clip}(w,M)^\perp\cdot g}\) \(+\) \(\lambda\sqrt{\mathrm{Var}(u)/n}\) |
...
\widehat R_i
=
\mathrm{DM}_i + \frac{\pi_\theta(a_i|x_i)}{\pi_b(a_i|x_i)}\,(r_i-\hat q_i),
\qquad
\mathrm{DM}_i = \sum_a \hat q(x_i,a)\,\pi_\theta(a|x_i).
\]

\[
\widehat R = \overline{\widehat R_i},
\quad
\mathrm{se} = \frac{s(\widehat R_i)}{\sqrt{n}},
\quad
...
\[
\widehat V_i = \mathrm{DM}_i + (r_i - \hat q_i),
\qquad
\mathrm{score} = \overline{\widehat V_i} - t_{0.975,\,n-1}\,\mathrm{se}(\widehat V).
\]
